...
function Eyebrow({ tag, label }) {
  ...
  return (
    <div className="flex items-center gap-x-3">
      {tag && <Tag>{tag}</Tag>}
      {tag && label && (
        <span className="h-0.5 w-0.5 bg-zinc-300 dark:bg-zinc-600" />
      )}
      {label && (
        <span className="min-w-0 font-mono text-xs text-zinc-500">{label}</span>
      )}
    </div>
  )
}
    ...
    >
      {inView && (
        <div className="absolute mt-1 ml-[calc(-1*var(--width))] hidden w-[var(--width)] opacity-0 transition [--width:calc(2.625rem+0.5px+50%-min(50%,calc(theme(maxWidth.lg)+theme(spacing.8))))] group-hover:opacity-100 group-focus:opacity-100 md:block lg:z-50 2xl:[--width:theme(spacing.10)]">
          <div className="group/anchor block h-5 w-5 bg-white ring-1 ring-inset ring-zinc-200 transition hover:ring-zinc-400 dark:bg-zinc-925 dark:ring-zinc-800 dark:hover:bg-zinc-900 dark:hover:ring-zinc-600">
            <AnchorIcon className="h-5 w-5 stroke-zinc-400 transition group-hover/anchor:stroke-zinc-600 dark:stroke-zinc-500 dark:group-hover/anchor:stroke-zinc-200" />
          </div>
        </div>
      )}
      {children}
