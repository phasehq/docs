...
- `host` type `sting`: The URL of the Phase Console instance
- `debug` type `bool`: Setting to true will results in higher level of log verbosity useful when debugging
...
Define key-value pairs and specify the Environment, App name, and path (optional) for each key to create new Secrets.
...
To retrieve the value of a single Secret, provide the Environment name, App name, Key, and optionally a tag and path.
