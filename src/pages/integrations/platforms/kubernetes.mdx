...
- Obtain a [`PHASE_SERVICE_TOKEN`](/console/tokens#service-tokens) for your application, that has access to the environment you are trying to fetch secrets from.
...
```fish
helm install phase-secrets-operator phase/phase-kubernetes-operator --set image.tag=v1.0.0
```
...
A Basic custom resource to sync all secrets from `production` environment in an app in the Phase Console to your Kubernetes cluster as `my-application-secret`, type `Opaque`.
...
metadata:
  ...
  namespace: default
spec:
  phaseAppEnv: 'production' # The Phase application environment to fetch secrets from
  phaseAppEnvTag: 'certs' # Tag for filtering secrets in the specified Phase app environment.
  phaseHost: 'https://console.phase.dev' # URL of a Phase Console instance
  authentication:
    serviceToken:
      serviceTokenSecretReference:
        secretName: 'phase-service-token' # Name of the Phase service token with access to your application
...
<Properties>
  <Property name="phaseAppEnv" type="optional">
    The Phase application environment to fetch secrets from (e.g., dev, staging,
    prod). <b>Default</b>: `prod`.
  </Property>
  ...
  <Property
    ...
    type="required"
  >
    The namespace of the Kubernetes managed secret containing your service
    token.
  </Property>
        ...
        - name: fetch-secrets
          ...
          command:
            [
              '/bin/sh',
              '-c',
              'phase secrets --app "my application name" --env development > /secrets/secrets.env',
            ]
      containers:
        - name: main-container
          image: alpine:latest
