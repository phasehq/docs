...
You can use Phase run to inject secrets to your application running to Hashicorp Nomad process during runtime.
...
In order to deploy secrets to our application running in a Nomad cluster we will use the Phase CLI Docker image, available at [Docker Hub](https://hub.docker.com/r/phasehq/cli/tags) configured as a [prestart](https://developer.hashicorp.com/nomad/docs/job-specification/lifecycle) container which will authenticate with the Phase service using the `PHASE_SERVICE_TOKEN`, fetch secrets and write them to a shared directory which our run-alpine task will use to consume the secrets.
...
job "my-application-deployment" {
  ...
  group "app" {
    ...
    task "fetch-secrets" {
      ...
      env {
        PHASE_SERVICE_TOKEN = "pss_service:v1:<your-applications-phase-service-token>"
