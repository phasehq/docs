...
### Create the `ACL Policy` via the Nomad web UI

Go to `Access Control` -> `Policies` -> `+ Create Policy`

Enter an desired adaptation of the above policy that suites your purpose and a descriptive policy name.
...
### Create the client token via the Nomad CLI:
...
### Create the `Client Token` via the Nomad web UI
