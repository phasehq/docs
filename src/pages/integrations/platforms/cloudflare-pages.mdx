...
<Note>
For security reasons Phase sets the secret type in Cloudflare Pages environment variables as type `Encrypted` for additional protection. This means that you will not be able to see the secret value in the Cloudflare Pages UI. You can still use the secret in your build commands and environment variables.
</Note>
...
![Go to integrations](/assets/images/platform-integrations/cloudflare-pages/integrations-sidebar.png)

![Add a credential](/assets/images/platform-integrations/cloudflare-pages/add-credential-button.png)
...
Go to your App in the Phase Console and go to the **Syncing** tab. Click **Enable Syncing** and then **Enable**.
...
<Note>
  If you see an error message and cannot proceed to the next screen, it is most likely an issue with the Account ID or Access Token. 
  Verify that you created the Cloudflare credentials correctly and update them from the Integrations screen if needed.
</Note>
