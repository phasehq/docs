...
```bash {{ title: 'Alpine Linux' }}
RUN apk add --no-cache bash sudo curl wget && curl -fsSL https://pkg.phase.dev/install.sh | bash
```

```bash {{ title: 'RedHat/CentOS/Amazon Linux' }}
...
ENTRYPOINT ["sh", "-c", "phase run yarn start"]
```
...
Certainly! Using the `ENTRYPOINT` in a Dockerfile to inject secrets via a tool like "phase" can have some benefits, but it also comes with its own set of security considerations. Let's discuss them:
