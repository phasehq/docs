...
az role assignment create \
  ...
  --assignee $(az ad signed-in-user show --query id -o tsv) \ # Replace with your 👇 resource group          and key vault 👇 
...
az ad sp create-for-rbac \
  --name phase-kv-sync \
  --role "Key Vault Secrets Officer" \ #  Replace with your resource group here 👇                         and key vault here 👇 
...
az ad sp create-for-rbac \
  --name phase-kv-sync \
  --role "Key Vault Secrets Officer" \ # Replace with your resource group here 👇  
  --scopes /subscriptions/$(az account show --query id -o tsv)/resourceGroups/phase-rg
```
...
<Note>
  ...
  service principal has the **Key Vault Secrets Officer** role on the target vault.
