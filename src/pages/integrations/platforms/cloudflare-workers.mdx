...
<Note>
Each time Phase syncs a secret to Cloudflare Worker, a new deployment is triggered. You will see several deployments in the Cloudflare Workers version history tab, with the source being labeled as `API`.
</Note>

You can use the following Cloudflare Worker API example to verify that the secrets are being synced correctly:
```js
/**
