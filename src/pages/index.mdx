...
- Easily share secrets with your [team](/console/users#add-users-to-an-organisation) and manage access to secrets with [cryptographic RBAC](/console/environments#manage-user-access).
- Sane defaults - out of the box [dev, staging, prod environments](/console/environments) setup, audit-logs, permissions etc.
...
You can replace .env files with runtime secret inject with a fully-featured [CLI](/cli/install), deploy secrets to third party services with native integrations, and build your own custom workflows and integrations with [SDKs](/sdks) and [APIs](/public-api). 
