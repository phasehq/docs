...
Phase implements GPG key signing for all commits to ensure the authenticity and integrity of the codebase. Commits can be verified against the GPG public keys of that of Phase maintainers and the trust chain of a particular git blame can be traced to it's origin

### Dependency Management and Scanning

We employ automated dependency updates and security scanning are managed through GitHub Dependabot. This system continuously monitors the project's dependencies across various languages for known vulnerabilities, automatically creating pull requests which are tested and merged based on their severity. This proactive approach helps maintain a secure and up-to-date codebase with minimal manual intervention, reducing the risk of exploitable vulnerabilities in third-party libraries lowering the overall attack surface.

### Public Container Builds

Phase's container images are built publicly in our repos via GitHub Actions, promoting transparency and allowing for community scrutiny of the build process. This open approach ensures that production containers are built from the exact code present in public repositories, enabling independent verification of the build pipeline.
