...
<Row>
  <Col>
    ...
    Soft-delete a service account. All associated tokens are immediately invalidated (subsequent requests with those tokens return `403 Forbidden`), and all app/environment access grants are removed. Subsequent operations on the deleted service account's ID return `404 Not Found`.
