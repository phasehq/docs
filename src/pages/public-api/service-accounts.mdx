...
<Row>
  <Col>
    ...
    Mint an additional bearer token for an existing service account. The server uses its keyring to generate the token end-to-end, so the caller only needs to supply a name and an optional TTL.
    ...
    - Expiry is expressed as a **relative** TTL in seconds via `expires_in`. The absolute `expires_at` / legacy `expiry` fields are explicitly rejected to avoid silently minting never-expiring tokens when a malformed value is supplied.
    ...
    <Properties>
      <Property name="expires_in" type="integer">
        Token lifetime in seconds (positive integer). Omit for a non-expiring token. Sending `expires_at` or `expiry` returns `400 Bad Request`.
      </Property>
    </Properties>
