...
<Row>
  <Col>
    ...
    - The role must not include permissions that the caller's own role does not have. Otherwise the request returns `403 Forbidden` and the error lists each extra permission. Callers with a global-access role are exempt.
