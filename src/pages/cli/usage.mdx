...
Alternatively, authenticate with Phase via a [Service Account Token](/access-control/service-accounts#create-a-new-service-account-token). This is suitable for using the CLI in a headless environment such as on your server, remote machine via SSH, Docker container, etc.
