...
- `--path`: (Optional) Specific path under which to fetch secrets from and inject into your application. Default is '/'
...
- `--path`: (Optional) The path in which you want to list secrets. Default is '/'
...
- `KEY`: (Optional) The key for the new secret. If the value is not provided as an argument, it will be read from stdin.
...
- `--path`: (Optional) The path from which you want to export secret(s). Default is '/'
- `--app`: (Optional) Name of your Phase application. Use this to override the `.phase.json` file or when it's not present in your project directory.
...
- `--format`: (Optional) Specify the export format. Supported formats are `dotenv`, `json`, `csv`, `yaml`, `xml`, `toml`, `hcl`, `ini`, `java_properties`. Default format is `dotenv`.
...
- The `export` command allows you to export secrets in various formats, making it flexible for different use cases.
