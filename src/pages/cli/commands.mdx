...
KEY 🗝️                | VALUE ✨
----------------------------------------------------------------------------------------
PROD_AWS_KEY         | 🌐` ${prod.AWS_ACCESS_KEY_ID}
...
- `--app`: (Optional) Name of your Phase application. Use this to override the `.phase.json` file or when it's not present in your project directory.
- `--app-id`: (Optional) ID of your Phase application. Takes precedence over `--app` if both are provided.
- `--random`: (Optional) Specify the type of random value to generate. Available types are `hex`, `alphanumeric`, `key128`, `key256`. Example: `--random hex`.
...
```fish
> phase secrets export [--env ENVIRONMENT] [--app APP_NAME] [--tags TAGS] [--format FORMAT]
```
