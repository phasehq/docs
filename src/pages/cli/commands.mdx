...
<Note>
  Note: If you are running the CLI in an headless environment (eg. CI/CD, Docker
  container, bash script) you can use pass the `PHASE_HOST` and
  `PHASE_SERVICE_TOKEN` environment variable. See [Environment
  Variable](commands#environment-variables) for more details.
</Note>
...
For `token` mode you will have to provide the following:
