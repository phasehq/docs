...
- `--no-login` (Optional): Use external indntity to generate and print a short-lived access token to STDOUT without logging into the CLI.
...
- `no-login` (Optional): Only applicable for when using external identities for authentication (e.g., `aws-iam` mode). Use external indntity to generate and print a short-lived access token to STDOUT without logging into the CLI.
