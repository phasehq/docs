...
<TabGroup title="Assume Role" subtitle="Choose a method of supplying AWS integration credentials to your Phase instance.">
...
<Note>
Please make sure the instance metadata service is enabled and IMDSv2 is enabled and reachable from within the containers.
</Note>
...
```
  ...
  <TabPanel title="Kubernetes ISRA" slug="isra">

If you are running Phase on an AWS EKS cluster, you can use an IAM role for service accounts to provision a machine role which Phase will use via the ISRA API to assume other roles to get temporary credentials. More information about this can be found [here](https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html).
...
1.  **Create the Trust Relationship JSON file:**
    ...
    ```
    *   **Note:** Replace `<YOUR_KUBERNETES_NAMESPACE>` with the Kubernetes namespace where Phase will be deployed (e.g., `default` or `phase-ns`). The service account name is `phase-global-assume-role` by default as per the Helm chart, adjust if you use a custom name.
...
If you are using the Phase Helm chart, you need to configure the service account to use the IAM role created in Step 2. This is done by annotating the service account with the IAM role ARN.
...
2.  After the user is created, AWS will return its details.
