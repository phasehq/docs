...
1. Create a new GitHub Oauth application. You may follow instructions [here](/access-control/authentication/oauth-sso#git-hub). Please note this is different from the one you may already have set up for GitHub SSO, which is used for authentication.
...
1. Create a new GitHub Enterprise Server Oauth application. You may follow instructions [here](/access-control/authentication/oauth-sso#git-hub). Please note this is different from the one you may already have set up for GitHub SSO, which is used for authentication.
2. Set the Authorization callback URL to: `https://[**YOUR_DOMAIN**]/service/oauth/github/callback`
3. Hit Register application
4. Supply the following secrets to your Phase Console instance
