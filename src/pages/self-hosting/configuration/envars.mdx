...
- `github-enterprise` - GitHub Enterprise OAuth 2.0
...
You can find instructions for setting up SSO with a GitHub Enterprise OAuth 2.0 App using the same steps as [GitHub OAuth](/access-control/authentication/oauth-sso#git-hub), but with your enterprise instance.

Set the "Authorization callback URL" for your GitHub Enterprise OAuth App as `${HTTP_PROTOCOL}${HOST}/api/auth/callback/github-enterprise`.
...
After setting up your App on GitHub Enterprise, set the base URL, API URL, Client ID and Client secret values:

<Properties>
  <Property name="GITHUB_ENTERPRISE_BASE_URL" type="string">
    The URL of your GitHub Enterprise instance. Example: `https://github.yourdomain.com`
  </Property>
  <Property name="GITHUB_ENTERPRISE_API_URL" type="string">
    The API URL of your GitHub Enterprise instance. This is usually GITHUB_ENTERPRISE_BASE_URL + /api. Example: `https://github.yourdomain.com/api`. Phase will reference the environment variable and construct the API URL of your instance as follows: `GITHUB_ENTERPRISE_API_URL/v3/user`.
  </Property>
  <Property name="GITHUB_ENTERPRISE_CLIENT_ID" type="string">
    GitHub Enterprise OAuth Client ID
  </Property>
  <Property name="GITHUB_ENTERPRISE_CLIENT_SECRET" type="string">
    GitHub Enterprise OAuth Client secret. Can be mounted from a file by suffixing `_FILE` to the key, pointing to a filepath.
  </Property>
</Properties>
...
1. Create a new GitHub Oauth application. Follow instructions [here](/access-control/authentication/oauth-sso#git-hub). (Note: This is different from the one you may already have set up for GitHub SSO)
...
1. Create a new GitHub Enterprise Oauth application. Follow instructions [here](/access-control/authentication/oauth-sso#git-hub). (Note: This is different from the one you may already have set up for GitHub SSO)
2. Set the Authorization callback URL to: `https://[**YOUR_DOMAIN**]/service/oauth/github/callback`
3. Hit Register application
4. Supply the following secrets to your Phase Console instance
...
<Properties>
  <Property name="GITHUB_ENTERPRISE_INTEGRATION_CLIENT_ID" type="string">
    GitHub Enterprise OAuth application Client ID

    Referenced by the [`frontend`](https://hub.docker.com/r/phasehq/frontend), [`backend`](https://hub.docker.com/r/phasehq/backend) and [`worker`](https://hub.docker.com/r/phasehq/backend) containers.
  </Property>
  <Property name="GITHUB_ENTERPRISE_INTEGRATION_CLIENT_SECRET" type="string">
    GitHub Enterprise OAuth application Client Secret. Can be mounted from a file by suffixing `_FILE` to the key, pointing to a filepath.
