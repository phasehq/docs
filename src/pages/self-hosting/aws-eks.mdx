...
If your Phase Console backend needs to access AWS services (e.g., S3, SES, Secrets Manager), you can use [IAM Roles for Service Accounts (IRSA)](https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html) to grant fine-grained IAM permissions to the Phase Console pods without managing AWS credentials.

### 1. Create an IAM Policy

Create an IAM policy with the permissions your Phase Console needs. For example, to allow sending emails via SES:
...
Use `eksctl` to create a service account with the IAM role:
...
The Helm chart will configure the backend, worker, and migration pods to use this service account, enabling them to assume the associated IAM role.
