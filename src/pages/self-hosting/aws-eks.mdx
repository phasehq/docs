...
This guide assumes you are setting up Phase Console with in-cluster databases. For external databases, you will need to adjust the Helm chart values accordingly. We will install cert-manager and an NGINX ingress controller. If you have these already set up in your EKS cluster, you may be able to adapt or skip some steps.

## Prerequisites

- An active AWS Account with necessary permissions to create EKS clusters, IAM roles, and related resources.
- [AWS CLI](https://aws.amazon.com/cli/) installed and configured.
- [eksctl](https://eksctl.io/installation/) installed.
- [kubectl](https://kubernetes.io/docs/tasks/tools/#kubectl) installed & configured.
...
metadata:
  name: phase-eks-cluster # You can change the cluster name if you like
...
After the cluster creation is complete, verify that `kubectl` is configured correctly:
...
With the EKS cluster, cert-manager, default StorageClass, and NGINX Ingress Controller configured, you are now ready to install Phase Console.
...
```
You may also need to manually delete related resources like PersistentVolumeClaims if they are not automatically removed, and clean up DNS records.

## Troubleshooting

If you encounter issues, particularly with TLS certificates:

If you visit your domain and see a self-signed certificate or errors after DNS propagation:
1.  Check the cert-manager pod logs in the `cert-manager` namespace.
2.  Check the logs of the `cm-acme-http-solver` pod that briefly appears to solve the Let's Encrypt challenge:
    ```fish
    ...
    ```
    Note that this pod is short-lived. If you don't see it, the certificate issuance might not be triggering correctly.
...
Ensure your domain's DNS records are correctly pointing to the NGINX LoadBalancer IP/hostname and that security groups or NACLs are not blocking traffic on ports 80/443 to the Load Balancer. 