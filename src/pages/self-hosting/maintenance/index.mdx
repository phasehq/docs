...
<Note>
To fully realize performance benefits and reclaim disk space, you will need to run `VACUUM` on the database after purging the logs. See [PostgreSQL documentation](https://www.postgresql.org/docs/current/app-vacuum.html) for more information.
</Note>
...
This will remove the dead rows and reclaim the space for future use. Depending on your table size, this operation can take anywhere from a few seconds to several minutes.
...
<Warning>
`VACUUM FULL` will lock your table, preventing writes to the `api_secretevent` table and significantly degrading read performance. During this period you will not be able to create secrets in Phase Console, CLI, SDKs or the API. Only run this command during maintenance windows when minimal activity is expected.
</Warning>
