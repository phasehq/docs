...
The performance impact on your Phase deployments and related assets depends entirely on the size the specifications of your database instance. For Phase instances with large number of audit-log events (100 million+), we recommend running the `purge_app_logs` command for the first time during off-peak hours new logs to avoid performance degradation. In our testing, it took about 2.5 minutes to delete 1 million read events on an AWS RDS `db.t3.micro` instance.

![AWS RDS](/assets/images/self-hosting/maintenance/rds-log-delete-graph.png)