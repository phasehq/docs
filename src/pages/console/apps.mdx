...
You may modify the one-liners to best suit your needs, by injecting, exporting, parsing the secrets via the CLI or over the REST API with curl. You may also run follow up commands as the oneliners set the Personal Access Token as the `PHASE_TOKEN` environment variable and the token is valid for 5 minutes.
