...
2. For older ranges: export the range from the [audit logs REST API](#audit-logs-rest-api) using the `start`/`end` parameters shown in the delivery row, and re-ingest it with your own tooling if needed. Deduplication on `event.id` makes re-ingestion safe. The REST API covers **organisation audit events**; expired **secret events** have no export endpoint yet, but always remain queryable in each app&apos;s Logs tab in the Console.
...
Stream logs to [Datadog](https://www.datadoghq.com/) for monitoring, alerting and Cloud SIEM detection. Works with all Datadog site regions (US1, US3, US5, EU1, AP1, AP2 and US1-FED).

### Step 1: Create a Datadog API key

1. Log in to Datadog and go to **Organization Settings** → **API Keys**.
2. Click **+ New Key** and name it, e.g. `phase-log-stream`. Copy the key value.
...
3. Note your Datadog **site** — you can see it in your browser&apos;s address bar, e.g. `us3.datadoghq.com`, `datadoghq.com`, or `datadoghq.eu`.

### Step 2: Add Datadog credentials to Phase

1. In the Phase Console, go to **Integrations** → **Third-party credentials**.
2. Click the **Datadog** card.
...
<Warning>
  ...
  hours**. If a stream is paused or failing for longer than that, the missed
  range is recorded as **skipped** in the delivery history, and the cursor
  jumps forward. Events always remain queryable in Phase — use the audit logs
  REST API below to export any range manually.
</Warning>
...
To manually re-ingest a range into Datadog, fetch it with the API above, map the fields to your preferred shape, and POST it to Datadog&apos;s [logs intake endpoint](https://docs.datadoghq.com/api/latest/logs/#send-logs) with your own API key.
