...
Outputs are read-only — Phase owns the value because it's auto-rotated. They can't be edited or shared, and don't accept personal overrides. Reveals are logged in the standard audit log.

References work as usual — `${ROTATED_KEY}` resolves to the current active credential.
...
- **Rotate now** — trigger an immediate rotation; the schedule resets to one interval from now.
