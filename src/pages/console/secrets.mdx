...
- The authentication medium (such as a token) must have appropriate access to all referenced secrets across relevant apps, environments, and paths. If the level of access is insufficient, the value of secret reference is not resolved and is returned as is with the reference syntax intact. Example: If you have access to `production` environment in `backend_api` application but not `production` in `frontend`, the value `${backend_api::production./frontend/SECRET_KEY}` will returned as is and without being resolved.

- Phase is unable to resolve the reference of a secret when a third party sync integration is enabled, it will fail  A secret sync integration will fail and not sync secrets if a particular secret is unable to be resolved. if any referenced secret does not exist locally, inside a path in an environment, or inside an app. This is by design to ensure secrets inside third-party platforms are never broken, as some third-party platforms support their own native secret referencing syntax.

- If two or more applications in your organization share the same name (case insensitive), the secret reference will be deemed ambiguous and will be returned as is without resolution over the REST API. 
