...
Secrets are key/value pairs used to store application secrets and configuration for your applications across [environments](/console/environments) in various folders and paths. Secrets in Phase can additionally have tags and comments associated with them. All changes to secrets in an environment automatically trigger deployments to their platform secret sync integrations. Every attempt to access, update or delete a secret will be logged and viewable on the [App logs](/console/apps#logs) screen.
...
<Note>
Secrets keys cannot be empty, or share the same key as another secret in the current environment at the same path.
</Note>
...
Then simply enter the value you wish to override this secret with. You can also control whether a Personal Secret override is active with the "Activate Secret Override" toggle switch.
...
You can easily share secrets to other by a secure link. When sharing a secret, you need to decide whether you simply want to link a secret to another user who already has access to it in Phase (via a Permalink) or an external user who may not have an account in Phase (via a Lockbox). You can hover over the secret row and click the "Share" button to see the menu to create a Permalink or Lockbox.

### Permalink

Permalinks allow you to share a secret with another users who already has access to it in Phase. This is useful when you want to link a specific secret inside of an App, Environment or Folder to others on your team.

![secret permalink](/assets/images/console/secrets/sharing/secret-permalink.png)

### Lockbox

Lockbox allows secrets to be shared via a single link with a [zero-trust encryption](/security/architecture#lockbox) scheme. The key that is used to decrypt the secret is encoded as a URL fragment and is not stored in Phase. When creating a Lockbox, you will be able to update or override the value of the secret and specify the expiry time and the maximum number of views. Please be mindful that anyone with the link will be able to access the secret.
...
To update key or the value of a secret, simply find the secret row of the secret you wish to update and start typing the new key or value. You may choose to reveal the value of the secret by clicking the "Reveal" button. Certain secrets values of boolean types can be easily flipped by clicking the switch button. The existing format of your boolean value will be preserved e.g., `True` will be flipped to `False` and vice versa, `true` will be flipped to `false` and so forth.

Secret that have been updated will be highlighted in yellow to indicate that they have been updated. You can update any number of secrets at once. Once finalized, click the "Deploy" button to save your changes.
