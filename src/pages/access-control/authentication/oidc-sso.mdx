...
2. Select "Microsoft Entra ID" from the side navigation blade or from Azure services.
...
3. In the Microsoft Entra ID dashboard, select "App registrations" from the Manage section in the sidebar.
...
5. Provide a name for your application, select the supported account types (who can use this application), and add the redirect URI for your Phase instance (e.g., https://[**YOUR_DOMAIN**].com/api/auth/callback/entra-id-oidc).
...
8. From the left sidebar, select "Certificates & secrets" under the Manage section.
...
10. Provide a description for your client secret and select an expiration period, then click "Add".
...
Your Microsoft Entra ID SSO integration is now configured! Users can now authenticate to your Phase instance using their Microsoft credentials.
