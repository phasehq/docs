...
1. The Phase CLI gets a signed `get-caller-identity` AWS Sigv4 request from AWS IAM and send it to Phase.
2. Phase validates the signature with AWS IAM, sees that the requesting clients principal ARN (e.g. `arn:aws:iam::123456789012:instance-profile/PhaseTrustedPrincipal`) has a trusted relationship with the Phase Service Account and returns a short-lived Service Account Token to the CLI.
3. The CLI uses this Service Account Token to fetch secrets and provision them into your application.
