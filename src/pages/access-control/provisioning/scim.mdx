...
### Step 1: Open your Phase OIDC application

SCIM provisioning is attached to the same OIDC app users sign in through — one app handles both authentication and lifecycle, which keeps assignments and group pushes in one place.

1. Sign in to the [Okta Admin Console](https://admin.okta.com).

2. Navigate to **Applications** > **Applications**.

3. Click on your existing Phase OIDC application (the one created in [Okta OIDC setup](/access-control/authentication/oidc-sso#okta)), then continue to Step 2.
...
- Verify the SCIM base URL ends with `/v1/scim/v2` (no trailing slash)
